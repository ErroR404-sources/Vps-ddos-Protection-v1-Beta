...
function akguy_banner() {
    cat << "EOF"
 𝘿𝘿𝙤𝙎 𝙋𝙧𝙤𝙩𝙚𝙘𝙩𝙞𝙤𝙣 𝙁𝙤𝙧 𝙑𝙈 𝙑𝙥𝙨 𝘽𝙮 404 𝙀𝙧𝙧𝙤𝙍
EOF
}
...
function server_hardening() {
    ...
    echo -e -n "${yellow}"
    echo -e "-------------------------------------------------- " | tee -a "$LOGFILE"
    echo -e " $(date +%m.%d.%Y_%H:%M:%S) : Botnet Destroyed By ErroR 404" | tee -a "$LOGFILE"
    echo -e "-------------------------------------------------- \n" | tee -a "$LOGFILE"
    echo -e -n "${lightcyan}"
    echo -e " The next steps are to secure your server's shared memory, enable"
    echo -e " DDOS protection,  networking layer and ssh layer, and enable automatic"
    ...
    then
        ...
        # enable DDOS protection By 404 ErroR
        echo -e -n "${yellow}"
        echo -e "---------------------------------------------------- " | tee -a "$LOGFILE"
...
function motd_install() {
    ...
    echo -e "--------------------------------------------------- \n" | tee -a "$LOGFILE"
    echo -e -n "${lightcyan}"
    echo -e " The normal 𝗠𝗼𝗿𝗲 𝗨𝗽𝗴𝗿𝗮𝗱𝗶𝗻𝗴 𝗧𝗵𝗲 𝗩𝗽𝘀 after a successful SSH login"
    ...
    # check if 𝗠𝗼𝗿𝗲 𝗨𝗽𝗴𝗿𝗮𝗱𝗶𝗻𝗴 𝗧𝗵𝗲 𝗩𝗽𝘀 is affirmative
    if [ "${MOTDP,,}" = "Y" ] || [ "${MOTDP,,}" = "y" ]
    then
...
function display_banner() {
    ...
    cat << "EOF"
 * * * * * * * * * * * * * * * * * * 
|| =) 404𝑬𝒓𝒓𝒐𝑹 𝑷𝒓𝒐𝒕𝒆𝒄𝒕𝒊𝒐𝒏 𝑭𝒐𝒓 𝑽𝒑𝒔 𝑽𝑴 || *p
 * * * * * * * * * * * * * * * * * *
EOF
    echo -e -n "${nocolor}"
}
